...
TITLE_FONT_SIZE = 60  # یا هر عددی که میخوای
DIM_FONT_SIZE = 80
THICK_FONT_SIZE = 100
...
pos_WT = ("center", "top")    # وسط بالا
pos_WB = ("center", "bottom") # وسط پایین
pos_HR = ("right", "center")  # راست وسط
pos_H  = ("left", "center")   # چپ وسط
...
# ویرایش تابع get_pos برای تنظیم دقیق‌تر فاصله‌ها
def get_pos(base_x, base_y, base_w, base_h, text_w, text_h, pos):
    x, y = base_x, base_y
    if pos[0] == "center":
        x = base_x + (base_w - text_w) // 2
    elif pos[0] == "left":
        x = base_x - text_w - 1   # فاصله افقی بیشتر شد
    elif pos[0] == "right":
        x = base_x + base_w + 1   # فاصله افقی بیشتر شد

    if pos[1] == "center":
        y = base_y + (base_h - text_h) // 2
    elif pos[1] == "top":
        y = max(MARGIN + 20, base_y - text_h - 12)  # فاصله عمودی بیشتر شد
    elif pos[1] == "bottom":
        y = min(OUT_H - MARGIN - text_h - 10, base_y + base_h + 12)
    return x, y
...
for idx, row in df_valid.iterrows():
    ...
    # رسم عنوان (فقط یک خط: Section Name)
    title_line = section  # فقط Section Name

    w, h = text_size(draw, title_line, title_font)
    x = (OUT_W - w) // 2
    y = TITLE_TOP_MARGIN
    draw.text((x, y), title_line, fill="black", font=title_font)

    content_top = y + h + 10  # فاصله با بقیه محتوا
    ...
    if TH_s:
        ...
        tw_th, th_th = text_size(draw, thick_label, thick_font_use)
        ...
        draw.text((thick_x, thick_y), thick_label, fill="black", font=thick_font_use)
    ...
    if WT_s:
        ...
        wt_x, wt_y = get_pos(img_x, img_y, new_w, new_h, wtw, wth, pos_WT)
    ...
    if WB_s:
        ...
        wb_x, wb_y = get_pos(img_x, img_y, new_w, new_h, wbw, wbh, pos_WB)
    ...
    if HR_s:
        ...
        hr_x, hr_y = get_pos(img_x, img_y, new_w, new_h, hrw, hrh, pos_HR)
    ...
    if H_s:
        ...
        h_x, h_y = get_pos(img_x, img_y, new_w, new_h, hw, hh, pos_H)
