...
TITLE_FONT_SIZE = 50  # یا هر عددی که میخوای
DIM_FONT_SIZE = 80
THICK_FONT_SIZE = 100
...
for idx, row in df_valid.iterrows():
    ...
    # رسم عنوان دو خطی (خط اول: کد، خط دوم: Section Name)
    title_line1 = ncode
    title_line2 = section

    w1, h1 = text_size(draw, title_line1, title_font)
    w2, h2 = text_size(draw, title_line2, title_font)

    x1 = (OUT_W - w1) // 2
    x2 = (OUT_W - w2) // 2

    y1 = TITLE_TOP_MARGIN
    draw.text((x1, y1), title_line1, fill="black", font=title_font)

    y2 = y1 + h1 + 5  # فاصله بین خطوط
    draw.text((x2, y2), title_line2, fill="black", font=title_font)

    content_top = y2 + h2 + 10  # فاصله با بقیه محتوا
