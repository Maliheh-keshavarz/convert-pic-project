...
FONT_PATH = None  # فونت دلخواه (مثلاً آدرس فونت فارسی اگر داری)
TITLE_FONT_SIZE = 68
DIM_FONT_SIZE = 40
THICK_FONT_SIZE = 60
...
for idx, row in df_valid.iterrows():
    ...
    # نام فایل بدون تغییر فاصله ها و فقط حذف نشانه های غیرمجاز
    ...
    # رسم خطوط متن عنوان یکی یکی و با فاصله کم از بالا (10 پیکسل از بالا)
    ...
    # ----------------- باز کردن قالب و کراپ حداکثر 5 پیکسل فضای سفید -----------------
    if tpl_path:
        try:
            tpl = Image.open(tpl_path).convert("RGBA")
            tpl_cropped = trim(tpl, border=20)

            # بزرگنمایی قالب (تا اندازه واقعی) یا مقیاس مناسب حداکثر بدون افت کیفیت زیاد
    ...
    if TH_s:
        ...
        # یک پس‌زمینه سفید برای خوانایی بهتر ضخامت
